...
$createOut = & curl.exe -sS -w ('@' + $formatFile.FullName) -X POST $authFlag $authArg -H 'Accept: application/vnd.github+json' -H 'Content-Type: application/json' -d ('@' + $bodyFile.FullName) $createUrl 2>&1
Remove-Item $formatFile -Force -ErrorAction SilentlyContinue
